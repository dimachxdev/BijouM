...
function enregistrerVente(){
  ...
  // Déduire du stock en grammes si applicable
  const poidsVente = local||importe||poids||0;
  if(poidsVente>0){
    // Chercher l'article stock correspondant (carat + provenance)
    const prov = local>0?'local':'importe';
    const stockItem = STATE.stock.find(function(s){
      return s.carat===carat && (s.type===prov||s.typeBijou===typeBijou) && s.poids>0;
    });
    if(stockItem){
      const unitesPoids = stockItem.poids>0 ? Math.round(poidsVente/stockItem.poids) : 0;
      if(unitesPoids>0 && stockItem.qty>=unitesPoids) stockItem.qty-=unitesPoids;
    }
  }
  STATE.ventes.unshift({id,date,client,description:desc,local,importe,poids:poidsVente,carat,typeBijou,paiement,montant,acompte,restant:montant-acompte,compteClientId,numFacture});
...
function renderSorties(){
  ...
  document.getElementById('btn-add-sortie').style.display=isAdmin()?'':'none';
  document.getElementById('sorties-perm-warning').style.display=isAdmin()?'none':'flex';
  document.getElementById('sorties-body').innerHTML=[...STATE.sorties].sort((a,b)=>b.date.localeCompare(a.date)).map(s=>`<tr><td style="white-space:nowrap;font-size:12px;color:var(--text-secondary)">${fmtDate(s.date)}</td><td>${s.typeBijou||s.ref||'—'}</td><td><span class="ref-code">${s.carat||'—'}</span></td><td style="text-align:center"><strong>${s.poids||s.qty||'—'}g</strong></td><td>${s.motif}${s.commentaire?` — <span style="color:var(--text-secondary)">${s.commentaire}</span>`:''}</td><td><span class="role-pill role-${s.validePar}">${s.validePar}</span></td></tr>`).join('');
}
function enregistrerSortie(){
  ...
  if(!date||!typeBijou||!motif||poidsSort<=0){showToast('Tous les champs sont obligatoires.');return;}
  ...
  STATE.sorties.unshift({id,date,typeBijou:labelType,carat:carat||'—',poids:poidsSort,motif,commentaire,validePar:'admin'});
  save();closeModal('modal-add-sortie');renderSorties();renderStocks();renderDashboard();
...
<div class="rpt-head"><h1>Rapport journalier — KAYOR</h1><div style="font-size:12px;color:#666">${fmtDateLong(date)} · Imprimé le ${new Date().toLocaleString('fr-FR')}</div></div>
${content}</body></html>`);
  win.document.close();
  setTimeout(()=>{win.focus();win.print();win.close();},500);
}
